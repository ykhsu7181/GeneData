...
assert.match(source, /首页 \/ Data \/ 课题组原始数据/, 'RawDataView should render the renamed breadcrumb')
assert.match(source, /课题组原始数据 Research Group Raw Data/, 'RawDataView should render the renamed title')
assert.match(source, /集中展示课题组原始测序数据、文件位置及校验状态。/, 'RawDataView should render the renamed subtitle')
