...
from files.query_views import (
    _parse_bed_lines,
    _parse_fasta_sequence_id,
    _parse_gff_lines,
    _resolve_chromosome_alias,
)
...
class ChromosomeAliasTests(SimpleTestCase):
    ...
    def test_fasta_parser_prefers_ori_sequence_id(self):
        self.assertEqual(
            _parse_fasta_sequence_id(">contig_01 description OriSeqID=Chr01 source=assembly"),
            "Chr01",
        )

    def test_fasta_parser_uses_first_token_without_ori_sequence_id(self):
        self.assertEqual(_parse_fasta_sequence_id(">Chr02 description"), "Chr02")

    def test_alias_resolver_accepts_raw_and_canonical_ids(self):
        self.assertEqual(_resolve_chromosome_alias("contig_01", self.aliases), "Chr01")
        self.assertEqual(_resolve_chromosome_alias("Chr01", self.aliases), "Chr01")

    def test_gff_filter_matches_raw_sequence_id_to_canonical_request(self):
        rows = _parse_gff_lines(
